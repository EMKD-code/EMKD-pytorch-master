...
def train(args):
    ...
    exp_name = 'sskd_student_{}_{}_weight{}+{}+{}+{}_T{}+{}+{}_ratio{}+{}_seed{}_{}_trial_{}'.format(\
                args.s_arch, args.depth,\
...
def search():
    ...
    x_t, y_t, z_t = block_depth[args.t_arch], block_depth[args.t_arch], block_depth[args.t_arch] # Teacher Arch
    x_s, y_s, z_s = block_depth[args.s_arch], block_depth[args.s_arch], block_depth[args.s_arch] # Student Arch

    print('Search for best student({}) architecture of teacher({}) using method({}), gpu-id={}\n'.format(args.t_arch, args.s_arch, args.distill, args.gpu_id))
    #fo = open('search.txt', 'a+')
    #fo.write('Search for best student({}) architecture of teacher({}) using method({})\n'.format(args.t_arch, args.s_arch, args.distill))
    #fo.close
    with open('search.txt', mode='a+') as f:
    ...
    while z >= 1 and y <= y_t:
        args.depth = str(x) + ',' + str(y) + ',' + str(z)
        if args.mc:
            args.B1_weight = original_depth / float(x)
            args.B2_weight = original_depth / float(y)
            args.B3_weight = original_depth / float(z)
        print("Searching:{}, architecture:{}".format(args.s_arch, args.depth))
        acc = train(args)
        # acc = random.randint(0, 100)
        print("Searching:{}, architecture:{}, acc:{}".format(args.s_arch, args.depth, acc))
        if acc > best_acc:
            best_acc = acc
            y_opt = y
            z_opt = z
    ...
    print("Teacher:{}, Student:{}, best architecture:{},{},{}, distill:{}, acc:{}".format(args.t_arch, args.s_arch, x_opt, y_opt, z_opt, args.distill, best_acc))
    #fo = open('search.txt', 'a+')
    with open('search.txt', mode='a+') as f:
        f.write("Teacher:{}, Student:{}, best architecture:{},{},{}, distill:{}, acc:{}\n".format(args.t_arch, args.s_arch, x_opt, y_opt, z_opt, args.distill, best_acc))
    #fo.close()
