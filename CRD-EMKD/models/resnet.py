...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("residual:", residual.shape[0], residual.shape[1], residual.shape[2], residual.shape[3])
        #print("out:", out.shape[0], out.shape[1], out.shape[2], out.shape[3])
        out += residual
        preact = out
        out = F.relu(out)
        if self.is_last:
...
class ResNetv2(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10):
        ...
        self.layer1 = self._make_layer(block, num_filters[1], n)
        self.layer2 = self._make_layer(block, num_filters[2], n, stride=2)
        self.layer3 = self._make_layer3(block, num_filters[3], n, stride=2)
        self.avgpool = nn.AvgPool2d(8)
        self.fc = nn.Linear(num_filters[3] * block.expansion, num_classes)
    ...
    def _make_layer3(self, block, planes, blocks, stride=1):
        ...
        layers = list([])
        blockv2 = BasicBlockv2
        layers.append(blockv2(self.inplanes, planes, stride, downsample, is_last=(blocks == 1)))
        
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(planes, self.inplanes * block.expansion,
                          kernel_size=1, stride=1, bias=False),
                nn.BatchNorm2d(self.inplanes * block.expansion),
            )
        layers.append(blockv2(planes, self.inplanes, stride=1, downsample=downsample, is_last=(blocks == 2)))
        ...
        layers.append(block(self.inplanes, planes, stride=1, downsample=downsample, is_last=(blocks == 3)))
        #self.inplanes = planes * block.expansion
...
def resnet20(**kwargs):
    return ResNet(20, [16, 16, 32, 64], 'basicblock', **kwargs)   # original
    # return ResNetv2(20, [16, 16, 32, 64], 'basicblock', **kwargs)     # for trial 2
