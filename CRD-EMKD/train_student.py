...
'''
def get_teacher_name_depth(model_path):
    segments = model_path.split('/')[-2].split('_')
    if segments[0][0:2] == "S:":
        segments[0] = segments[0][2:]
    if segments[0] != 'wrn' and segments[0] != 'mywrn':
        depth = list([])
        depth.append(int(segments[1]))
        depth.append(int(segments[2]))
        depth.append(int(segments[3]))
        return segments[0], depth  # S:myresnetxx...
        #return segments[0], depth   #myresnetxx...
    else:
        depth = list([])
        depth.append(int(segments[3]))
        depth.append(int(segments[4]))
        depth.append(int(segments[5]))
        return segments[0] + '_' + segments[1] + '_' + segments[2], depth
'''

def remove_module_in_state_dict(filepath):
	state_dict = torch.load(filepath)['model']
...
'''
def load_my_teacher(model_path, n_cls):
    print('==> loading teacher model')
    model_t, block_depth = get_teacher_name_depth(model_path)
    #print(model_t, block_depth)
    model = model_dict[model_t](num_classes=n_cls, block_depth=block_depth)
    model.load_state_dict(torch.load(model_path)['model'])
    #model.load_state_dict(remove_module_in_state_dict(model_path))
    print('==> done')
    return model
...
def main():
    ...
    # routine
    log_path = os.path.join('./save/log', opt.model_name)
    if not os.path.isdir(log_path):
        os.makedirs(log_path)
    fo = open(log_path + '/log.txt', 'a')
    fo.write('epoch,train_acc,train_loss,test_acc,test_loss,test_acc_top5\n')
    for epoch in range(1, opt.epochs + 1):
        ...
        fo.write('{},{},{},{},{},{}\n'.format(epoch, train_acc, train_loss, test_acc, test_loss, test_acc_top5))
